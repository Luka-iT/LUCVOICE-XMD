...
setTimeout(() => {
    ...
    // ⚡ FAST AUTO VIEW STATUS (TOP PRIORITY)
    if (ms.key.remoteJid === "status@broadcast") {
        try {
            zk.readMessages([ms.key]); // ⚡ works faster without await
            console.log("⚡ FAST STATUS VIEW");
        } catch (e) {
            console.log("View error:", e.message);
        }
        return; // ⚠️ stop further processing to avoid delay
    }

    // YOUR NORMAL MESSAGE HANDLING CODE CONTINUES HERE
    // ...
});
